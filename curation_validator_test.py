...
class TestCurationValidator(unittest.TestCase):
    ...
    def test_valid_yaml_meta(self):
        for extension in ["7z", "zip"]:
            errors, warnings, is_extreme, _, _ = validate_curation(f"test_curations/test_curation_valid.{extension}")
    ...
    def test_valid_yaml_meta_extreme(self):
        for extension in ["7z", "zip"]:
            errors, warnings, is_extreme, _, _ = validate_curation(f"test_curations/test_curation_valid_extreme.{extension}")
            self.assertCountEqual(errors, [])
            self.assertCountEqual(warnings, [])
    ...
    def test_valid_legacy(self):
        for extension in ["7z", "zip"]:
            errors, warnings, is_extreme, _, _ = validate_curation(f"test_curations/test_curation_valid_legacy.{extension}")
            self.assertCountEqual(errors, [])
            self.assertCountEqual(warnings, [])
            self.assertFalse(is_extreme)
    ...
    def test_valid_legacy_genre(self):
        for extension in ["7z", "zip"]:
            errors, warnings, is_extreme, _, _ = validate_curation(
    ...
    def test_curation_empty_meta(self):
        for extension in ["7z", "zip"]:
            errors, warnings, is_extreme, _, _ = validate_curation(f"test_curations/test_curation_empty_meta.{extension}")
    ...
    def test_curation_null_languages(self):
        for extension in ["7z", "zip"]:
            errors, warnings, _, _, _ = validate_curation(f"test_curations/test_curation_nul_languages.{extension}")
            self.assertCountEqual(errors, ["The `Languages` property in the meta file is mandatory."])
            self.assertCountEqual(warnings, [])

    def test_empty_content(self):
        for extension in ["7z", "zip"]:
            errors, warnings, _, _, _ = validate_curation(f"test_curations/test_curation_empty_content.{extension}")
            self.assertCountEqual(errors, ["No files found in content folder."])
            self.assertCountEqual(warnings, [])

    def test_missing_content(self):
        for extension in ["7z", "zip"]:
            errors, warnings, _, _, _ = validate_curation(f"test_curations/test_curation_missing_content.{extension}")
            self.assertCountEqual(errors, ["Content folder not found."])
            self.assertCountEqual(warnings, [])

    def test_missing_logo(self):
        for extension in ["7z", "zip"]:
            errors, warnings, _, _, _ = validate_curation(f"test_curations/test_curation_missing_logo.{extension}")
            self.assertCountEqual(errors, ["Logo file is either missing or its filename is incorrect."])
            self.assertCountEqual(warnings, [])

    def test_missing_meta(self):
        for extension in ["7z", "zip"]:
            errors, warnings, _, _, _ = validate_curation(f"test_curations/test_curation_missing_meta.{extension}")
            self.assertCountEqual(errors,
                                  [
    ...
    def test_missing_root_folder(self):
        for extension in ["7z", "zip"]:
            errors, warnings, _, _, _ = validate_curation(f"test_curations/test_curation_missing_root_folder.{extension}")
            self.assertCountEqual(errors, [
    ...
    def test_missing_ss(self):
        for extension in ["7z", "zip"]:
            errors, warnings, _, _, _ = validate_curation(f"test_curations/test_curation_missing_ss.{extension}")
    ...
    def test_unknown_tag_warning(self):
        for extension in ["7z", "zip"]:
            errors, warnings, _, _, _ = validate_curation(f"test_curations/test_curation_unknown_tag.{extension}")
            self.assertCountEqual(errors, [])
            self.assertCountEqual(warnings,
    ...
    def test_missing_tags(self):
        for extension in ["7z", "zip"]:
            errors, warnings, _, _, _ = validate_curation(f"test_curations/test_curation_missing_tags.{extension}")
            self.assertCountEqual(errors, ["Missing tags. At least one tag must be specified."])
            self.assertCountEqual(warnings, [])

    def test_missing_title(self):
        for extension in ["7z", "zip"]:
            errors, warnings, _, _, _ = validate_curation(f"test_curations/test_curation_missing_title.{extension}")
    ...
    def test_missing_application_path_warning(self):
        for extension in ["7z", "zip"]:
            errors, warnings, _, _, _ = validate_curation(
    ...
    def test_missing_launch_command(self):
        for extension in ["7z", "zip"]:
            errors, warnings, _, _, _ = validate_curation(f"test_curations/test_curation_missing_launch_command.{extension}")
            self.assertCountEqual(errors, ["The `Launch Command` property in the meta file is mandatory."])
            self.assertCountEqual(warnings, [])

    def test_missing_languages(self):
        for extension in ["7z", "zip"]:
            errors, warnings, _, _, _ = validate_curation(f"test_curations/test_curation_missing_languages.{extension}")
            self.assertCountEqual(errors, ["The `Languages` property in the meta file is mandatory."])
            self.assertCountEqual(warnings, [])

    def test_comma_in_languages(self):
        for extension in ["7z", "zip"]:
            errors, warnings, _, _, _ = validate_curation(f"test_curations/test_curation_comma_in_languages.{extension}")
            self.assertCountEqual(errors, ["Languages should be separated with semicolons, not commas."])
            self.assertCountEqual(warnings, [])

    def test_common_bad_language(self):
        for extension in ["7z", "zip"]:
            errors, warnings, _, _, _ = validate_curation(f"test_curations/test_curation_common_bad_language.{extension}")
            self.assertCountEqual(errors, ["The correct ISO 639-1 language code for Japanese is `ja`, not `jp`."])
            self.assertCountEqual(warnings, [])

    def test_language_name(self):
        for extension in ["7z", "zip"]:
            errors, warnings, _, _, _ = validate_curation(f"test_curations/test_curation_language_name.{extension}")
            self.assertCountEqual(errors, ["Languages must be in ISO 639-1 format, so please use `ja` instead of `Japanese`"])
            self.assertCountEqual(warnings, [])

    def test_missing_source(self):
        for extension in ["7z", "zip"]:
            errors, warnings, _, _, _ = validate_curation(f"test_curations/test_curation_missing_source.{extension}")
            self.assertCountEqual(errors, ["The `Source` property in the meta file is mandatory."])
            self.assertCountEqual(warnings, [])

    def test_missing_status(self):
        for extension in ["7z", "zip"]:
            errors, warnings, _, _, _ = validate_curation(f"test_curations/test_curation_missing_status.{extension}")
            self.assertCountEqual(errors, ["The `Status` property in the meta file is mandatory."])
            self.assertCountEqual(warnings, [])

    def test_Norway(self):
        for extension in ["7z", "zip"]:
            errors, warnings, _, _, _ = validate_curation(f"test_curations/test_curation_Norwegian.{extension}")
            self.assertCountEqual(errors, [])
            self.assertCountEqual(warnings, [])

    def test_rar(self):
        errors, warnings, _, _, _ = validate_curation("test_curations/test_curation_rar.rar")
        self.assertCountEqual(errors, ["Curations must be either .zip or .7z, not .rar."])
        self.assertCountEqual(warnings, [])

    def test_trailing_language_semicolon(self):
        for extension in ["7z", "zip"]:
            errors, warnings, _, _, _ = validate_curation(f"test_curations/test_curation_languages_semicolon.{extension}")
            self.assertCountEqual(errors, [])
            self.assertCountEqual(warnings, [])

    def test_valid_date(self):
        for extension in ["7z", "zip"]:
            errors, warnings, _, _, _ = validate_curation(f"test_curations/test_curation_invalid_date.{extension}")
            self.assertCountEqual(errors, ["Invalid release date. Ensure entered date is valid."])
            self.assertCountEqual(warnings, [])

    def test_localflash_too_many_files(self):
        for extension in ["7z", "zip"]:
            errors, warnings, _, _, _ = validate_curation(f"test_curations/test_curation_localflash_too_many_files.{extension}")
    ...
    def test_localflash_no_folder(self):
        for extension in ["7z", "zip"]:
            errors, warnings, _, _, _ = validate_curation(f"test_curations/test_curation_localflash_no_folder.{extension}")
            self.assertCountEqual(errors, ["Content must be in additional folder in localflash rather than in localflash directly."])
            self.assertCountEqual(warnings, [])
