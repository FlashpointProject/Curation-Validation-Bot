...
def validate_curation(filename: str) -> tuple[list, list, Optional[bool], Optional[CurationType], Optional[str]]:
    errors: list = []
    warnings: list = []
    ...
    if len(meta) == 0:
        ...
        if extreme[1] and (props["Extreme"] == "Yes" or props["Extreme"] is True):
            is_extreme = True
        if props.get("Library") == "theatre":
            curation_type = CurationType.ANIMATION
        else:
            platform = props.get("Platform")
            if platform == "Flash" or platform is None:
    ...
    archive_cleanup(filename, base_path)
    # noinspection PyUnboundLocalVariable
    return errors, warnings, is_extreme, curation_type, props.get("Title")
